...
from graphql.type import (
    GraphQLArgument, GraphQLBoolean, GraphQLEnumType,
    GraphQLEnumValue, GraphQLField, GraphQLFloat,
    GraphQLID, GraphQLInputField,
...
Dog = GraphQLObjectType('Dog', {
    ...
    'isHousetrained': GraphQLField(
        GraphQLBoolean,
        args={'atOtherHomes': GraphQLArgument(
            GraphQLBoolean, default_value=True)}),
    'isAtLocation': GraphQLField(
        GraphQLBoolean,
        args={'x': GraphQLArgument(GraphQLInt),
              'y': GraphQLArgument(GraphQLInt)})},
    interfaces=[Being, Pet, Canine], is_type_of=lambda: True)
...
Cat = GraphQLObjectType('Cat', lambda: {
    ...
    'name': GraphQLField(GraphQLString, {
        'surname': GraphQLArgument(GraphQLBoolean)}),
    'nickname': GraphQLField(GraphQLString)},
    interfaces=[Being, Pet], is_type_of=lambda: True)
...
ComplicatedArgs = GraphQLObjectType('ComplicatedArgs', {
    ...
    'stringListNonNullArgField': GraphQLField(GraphQLString, args={
    ...
    'nonNullFieldWithDefault': GraphQLField(GraphQLString, {
...
AnyScalar = GraphQLScalarType(
    ...
    parse_value=lambda value: value)   # Allows any value
...
QueryRoot = GraphQLObjectType('QueryRoot', {
    ...
    'invalidArg': GraphQLField(GraphQLString, args={
        'arg': GraphQLArgument(InvalidScalar)}),
    'anyArg': GraphQLField(GraphQLString, args={
        'arg': GraphQLArgument(AnyScalar)})})
