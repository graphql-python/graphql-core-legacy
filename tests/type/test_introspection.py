...
def describe_introspection():
    ...
    def introspects_on_input_object():
        ...
        TestType = GraphQLObjectType('TestType', {
            'field': GraphQLField(GraphQLString, args={
                'complex': GraphQLArgument(TestInputObject)},
    ...
    def fails_as_expected_on_the_type_root_field_without_an_arg():
        ...
        request = """
            ...
            """
        assert graphql_sync(schema, request) == (None, [{
            'message': missing_field_arg_message(
                '__type', 'name', 'String!'), 'locations': [(3, 15)]}])
    ...
    def exposes_descriptions_on_enums():
        ...
        assert graphql_sync(schema, request) == ({
            'typeKindType': {
                'name': '__TypeKind',
                'description':
                    'An enum describing what kind of type'
                    ' a given `__Type` is.',
                'enumValues': [{
