...
from graphql.type import (
    GraphQLArgument, GraphQLBoolean, GraphQLField,
    GraphQLInt, GraphQLString, GraphQLObjectType, GraphQLList,
    GraphQLScalarType, GraphQLInterfaceType, GraphQLUnionType,
    GraphQLEnumType, GraphQLEnumValue, GraphQLInputObjectType, GraphQLSchema,
    GraphQLOutputType, GraphQLInputField, GraphQLNonNull, is_input_type,
    is_output_type)
...
def describe_type_system_example():
    ...
    def defines_an_enum_type_with_a_value_of_none_and_invalid():
        EnumTypeWithNullishValue = GraphQLEnumType(
            ...
            values={'NULL': None, 'UNDEFINED': INVALID})
    ...
    def defines_an_object_type_with_deprecated_field():
        TypeWithDeprecatedField = GraphQLObjectType('foo', {
    ...
    def includes_nested_input_objects_in_the_map():
        ...
        SomeInputObject = GraphQLInputObjectType('SomeInputObject', {
            'nested': GraphQLInputField(NestedInputObject)})
        SomeMutation = GraphQLObjectType('SomeMutation', {
            'mutateSomething': GraphQLField(BlogArticle, {
                'input': GraphQLArgument(SomeInputObject)})})
        SomeSubscription = GraphQLObjectType('SomeSubscription', {
            'subscribeToSomething': GraphQLField(BlogArticle, {
                'input': GraphQLArgument(SomeInputObject)})})
    ...
    def does_not_mutate_passed_field_definitions():
        fields = {
            ...
            'field2': GraphQLField(GraphQLString, args={
                'id': GraphQLArgument(GraphQLString)})}
...
def describe_field_config_must_be_a_dict():
    ...
    def accepts_an_object_type_with_a_field_function():
        obj_type = GraphQLObjectType('SomeObject', lambda: {
    ...
    def rejects_an_object_type_field_function_that_returns_incorrect_type():
        obj_type = GraphQLObjectType('SomeObject',
                                     lambda: [GraphQLField(GraphQLString)])
        with raises(TypeError) as exc_info:
            if obj_type.fields:
                pass
        msg = str(exc_info.value)
...
def describe_field_args_must_be_a_dict():
    ...
    def accepts_an_object_type_with_field_args():
        obj_type = GraphQLObjectType('SomeObject', {
            'goodField': GraphQLField(GraphQLString, args={
    ...
    def does_not_accept_is_deprecated_as_argument():
        ...
        with raises(TypeError) as exc_info:
            GraphQLObjectType('OldObject', {
                'field': GraphQLField(GraphQLString, **kwargs)})
        msg = str(exc_info.value)
...
def describe_object_interfaces_must_be_a_sequence():
    ...
    def accepts_an_object_type_with_list_interfaces():
        obj_type = GraphQLObjectType(
            'SomeObject', interfaces=[InterfaceType],
    ...
    def accepts_object_type_with_interfaces_as_a_function_returning_a_list():
        obj_type = GraphQLObjectType(
            'SomeObject', interfaces=lambda: [InterfaceType],
            fields={'f': GraphQLField(GraphQLString)})
        assert obj_type.interfaces == [InterfaceType]
...
def describe_type_system_object_fields_must_have_valid_resolve_values():
    ...
    def rejects_a_constant_scalar_value_resolver():
        ...
        msg = str(exc_info.value)
        assert msg == (
            'Field resolver must be a function if provided,  but got: 0.')


def describe_type_system_interface_types_must_be_resolvable():

    def accepts_an_interface_type_defining_resolve_type():
        AnotherInterfaceType = GraphQLInterfaceType('AnotherInterface', {
            'f': GraphQLField(GraphQLString)})

        schema = schema_with_field_type(GraphQLObjectType('SomeObject', {
            'f': GraphQLField(GraphQLString)}, [AnotherInterfaceType]))

        assert schema.query_type.fields[
            'field'].type.interfaces[0] is AnotherInterfaceType

    def rejects_an_interface_type_with_an_incorrect_type_for_resolve_type():
        with raises(TypeError) as exc_info:
            # noinspection PyTypeChecker
            GraphQLInterfaceType('AnotherInterface', {
                'f': GraphQLField(GraphQLString)}, resolve_type={})
        msg = str(exc_info.value)
        assert msg == (
...
def describe_type_system_union_types_must_be_resolvable():
    ...
    ObjectWithIsTypeOf = GraphQLObjectType('ObjectWithIsTypeOf', {
        'f': GraphQLField(GraphQLString)})
    ...
    def accepts_a_union_of_object_types_defining_is_type_of():
        schema_with_field_type(GraphQLUnionType(
            'SomeUnion', [ObjectWithIsTypeOf]))

    def rejects_an_interface_type_with_an_incorrect_type_for_resolve_type():
        with raises(TypeError) as exc_info:
            # noinspection PyTypeChecker
            schema_with_field_type(GraphQLUnionType(
                'SomeUnion', [ObjectWithIsTypeOf], resolve_type={}))
        msg = str(exc_info.value)
        assert msg == (
            "SomeUnion must provide 'resolve_type' as a function,"
            ' but got: {}.')
...
def describe_type_system_scalar_types_must_be_serializable():
    ...
    def accepts_a_scalar_type_defining_parse_value_and_parse_literal():
        schema_with_field_type(GraphQLScalarType(
            'SomeScalar', serialize=lambda: None,
            parse_value=lambda: None, parse_literal=lambda: None))

    def rejects_a_scalar_type_defining_parse_value_but_not_parse_literal():
        with raises(TypeError) as exc_info:
            schema_with_field_type(GraphQLScalarType(
                'SomeScalar', lambda: None, parse_value=lambda: None))
        msg = str(exc_info.value)
...
def describe_type_system_object_types_must_be_assertable():
    ...
    def accepts_an_object_type_with_an_is_type_of_function():
        schema_with_field_type(GraphQLObjectType('AnotherObject', {
            'f': GraphQLField(GraphQLString)}))
    ...
    def rejects_an_object_type_with_an_incorrect_type_for_is_type_of():
        with raises(TypeError) as exc_info:
            # noinspection PyTypeChecker
            schema_with_field_type(GraphQLObjectType('AnotherObject', {
                'f': GraphQLField(GraphQLString)}, is_type_of={}))
        msg = str(exc_info.value)
        assert msg == (
...
def describe_type_system_input_objects_must_have_fields():
    ...
    def accepts_an_input_object_type_with_fields():
        input_obj_type = GraphQLInputObjectType('SomeInputObject', {
            'f': GraphQLInputField(GraphQLString)})
...
def describe_type_system_enum_types_must_be_well_defined():
    ...
    def accepts_a_well_defined_enum_type_with_internal_value_definition():
        ...
        enum_type = GraphQLEnumType('SomeEnum', {
    ...
    def rejects_an_enum_type_with_incorrectly_typed_values():
        ...
        msg = str(exc_info.value)
        assert msg == (
            'SomeEnum values must be an Enum'
            ' or a dict with value names as keys.')

    def does_not_allow_is_deprecated():
        with raises(TypeError) as exc_info:
            # noinspection PyArgumentList
            GraphQLEnumType('SomeEnum', {
                'FOO': GraphQLEnumValue(is_deprecated=True)})
        msg = str(exc_info.value)
        assert "got an unexpected keyword argument 'is_deprecated'" in msg
...
def describe_type_system_list_must_accept_only_types():
    ...
    types = [
        GraphQLString, ScalarType, ObjectType,
        UnionType, InterfaceType, EnumType, InputObjectType,
        GraphQLList(GraphQLString), GraphQLNonNull(GraphQLString)]
...
def describe_type_system_non_null_must_only_accept_non_nullable_types():
    ...
    nullable_types = [
        GraphQLString, ScalarType, ObjectType,
        UnionType, InterfaceType, EnumType, InputObjectType,
        GraphQLList(GraphQLString), GraphQLList(GraphQLNonNull(GraphQLString))]

    not_nullable_types = [
        GraphQLNonNull(GraphQLString), {}, dict, str, object, None]
...
def describe_type_system_a_schema_must_contain_uniquely_named_types():
    ...
    def rejects_a_schema_with_same_named_objects_implementing_an_interface():
        ...
        FirstBadObject = GraphQLObjectType(
            'BadObject', {'f': GraphQLField(GraphQLString)},
