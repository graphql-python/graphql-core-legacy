...
from graphql.type import (
    ...
    GraphQLInputField, GraphQLInputObjectType, GraphQLList, GraphQLNonNull,
    GraphQLObjectType, GraphQLScalarType, GraphQLSchema, GraphQLString)

TestComplexScalar = GraphQLScalarType(
    name='ComplexScalar',
    serialize=lambda value:
        'SerializedValue' if value == 'DeserializedValue' else None,
    parse_value=lambda value:
        'DeserializedValue' if value == 'SerializedValue' else None,
    parse_literal=lambda ast, _variables=None:
        'DeserializedValue' if ast.value == 'SerializedValue' else None)
...
TestType = GraphQLObjectType('TestType', {
    ...
    'fieldWithNonNullableEnumInput': field_with_input_arg(GraphQLArgument(
        GraphQLNonNull(TestEnum))),
    'fieldWithObjectInput': field_with_input_arg(GraphQLArgument(
        TestInputObject)),
    'fieldWithNullableStringInput': field_with_input_arg(GraphQLArgument(
        GraphQLString)),
    'fieldWithNonNullableStringInput': field_with_input_arg(GraphQLArgument(
        GraphQLNonNull(GraphQLString))),
    'fieldWithDefaultArgumentValue': field_with_input_arg(GraphQLArgument(
        GraphQLString, default_value='Hello World')),
    ...
    'list': field_with_input_arg(GraphQLArgument(
        GraphQLList(GraphQLString))),
    'nnList': field_with_input_arg(GraphQLArgument(
        GraphQLNonNull(GraphQLList(GraphQLString)))),
    'listNN': field_with_input_arg(GraphQLArgument(
        GraphQLList(GraphQLNonNull(GraphQLString)))),
    'nnListNN': field_with_input_arg(GraphQLArgument(
...
def describe_execute_handles_inputs():
    ...
    def describe_handles_objects_and_nullability():
        ...
        def describe_using_inline_struct():
            ...
            def does_not_use_incorrect_value():
                ...
                assert result == ({'fieldWithObjectInput': None}, [{
            ...
            def properly_runs_parse_literal_on_complex_scalar_types():
                ...
                assert result == ({
                    'fieldWithObjectInput':
                        "{'c': 'foo', 'd': 'DeserializedValue'}"}, None)
        ...
        def describe_using_variables():
            ...
            def uses_null_when_variable_provided_explicit_null_value():
                ...
                assert result == (
                    {'fieldWithNullableStringInput': 'None'}, None)
            ...
            def does_not_use_default_value_when_provided():
                ...
                assert result == (
                    {'fieldWithNullableStringInput': "'Variable value'"}, None)
            ...
            def errors_on_addition_of_unknown_input_field():
                ...
                assert result == (None, [{
                    'message':
                        "Variable '$input' got invalid value {'a': 'foo',"
    ...
    def describe_handles_non_nullable_scalars():
        ...
        def allows_non_nullable_inputs_to_be_omitted_given_a_default():
            ...
            assert result == ({
                'fieldWithNonNullableStringInput': "'default'"}, None)
        ...
        def reports_error_for_missing_non_nullable_inputs():
            ...
            assert result == ({'fieldWithNonNullableStringInput': None}, [{
    ...
    def describe_execute_uses_argument_default_values():
        ...
        def when_no_argument_provided():
            ...
            assert result == ({
                'fieldWithDefaultArgumentValue': "'Hello World'"}, None)
