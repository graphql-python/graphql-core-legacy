...
def describe_execute_handles_basic_execution_tasks():
    ...
    def accepts_an_object_with_named_properties_as_arguments():
        ...
        schema = GraphQLSchema(GraphQLObjectType('Type', {
            'a': GraphQLField(GraphQLString,
                              resolve=lambda root_value, *args: root_value)}))
    ...
    def executes_arbitrary_code():
        ...
        DataType = GraphQLObjectType('DataType', lambda: {
            ...
            'pic': GraphQLField(GraphQLString, args={
                'size': GraphQLArgument(GraphQLInt)},
    ...
    def merges_parallel_fragments():
        ...
        assert execute(schema, ast) == ({
            'a': 'Apple', 'b': 'Banana', 'c': 'Cherry', 'deep': {
                'b': 'Banana', 'c': 'Cherry', 'deeper': {
                    'b': 'Banana', 'c': 'Cherry'}}}, None)
    ...
    def provides_info_about_current_execution_state():
        ...
        schema = GraphQLSchema(GraphQLObjectType('Test', {
        ...
        assert infos[0] == GraphQLResolveInfo(
            ...
            path=ResponsePath(None, 'result'), schema=schema,
            fragments={}, root_value=root_value, operation=operation,
            variable_values={'var': 'abc'}, context=None)
    ...
    def threads_root_value_context_correctly():
        ...
        schema = GraphQLSchema(GraphQLObjectType('Type', {
            'a': GraphQLField(GraphQLString, resolve=resolve)}))
    ...
    def correctly_threads_arguments():
        ...
        schema = GraphQLSchema(GraphQLObjectType('Type', {
            'b': GraphQLField(GraphQLString, args={
    ...
    def nulls_out_error_subtrees():
        ...
        assert execute(schema, ast, Data()).get() == ({
            ...
            'locations': [(3, 15)], 'path': ['syncError']}, {
            'message': 'Error getting syncRawError',
            'locations': [(4, 15)], 'path': ['syncRawError']}, {
            'message': 'Error getting syncReturnError',
            'locations': [(5, 15)], 'path': ['syncReturnError']}, {
            'message': 'Error getting syncReturnErrorList1',
            'locations': [(6, 15)], 'path': ['syncReturnErrorList', 1]}, {
            'message': 'Error getting syncReturnErrorList3',
            'locations': [(6, 15)], 'path': ['syncReturnErrorList', 3]}, {
            'message': 'Error getting asyncError',
            'locations': [(8, 15)], 'path': ['asyncError']}, {
            'message': 'Error getting asyncRawError',
            'locations': [(9, 15)], 'path': ['asyncRawError']}, {
            'message': 'Error getting asyncReturnError',
            'locations': [(10, 15)], 'path': ['asyncReturnError']}, {
    ...
    def full_response_path_is_included_for_non_nullable_fields():
        ...
        assert execute(schema, parse(query)) == ({
            ...
            'locations': [(7, 23)], 'path': [
                'nullableA', 'aliasedA', 'nonNullA', 'anotherA', 'throws']
        }])
    ...
    def provides_error_if_no_operation_is_provided():
        ...
        assert execute(schema, ast, Data()) == (None, [{
    ...
    def errors_if_unknown_operation_name_is_provided():
        ...
        assert execute(schema, ast, operation_name='UnknownExample') == (
            None, [{'message': "Unknown operation named 'UnknownExample'."}])
    ...
    def uses_the_query_schema_for_queries():
        ...
        assert execute(schema, ast, Data(), operation_name='Q') == (
            {'a': 'b'}, None)
    ...
    def uses_the_mutation_schema_for_mutations():
        ...
        assert execute(schema, ast, Data(), operation_name='M') == (
            {'c': 'd'}, None)
    ...
    def uses_the_subscription_schema_for_subscriptions():
        ...
        schema = GraphQLSchema(
            query=GraphQLObjectType(
                'Q', {'a': GraphQLField(GraphQLString)}),
            subscription=GraphQLObjectType(
                'S', {'a': GraphQLField(GraphQLString)}))
    ...
    def fails_when_an_is_type_of_check_is_not_met():
        ...
        assert execute(schema, query, value) == ({
            ...
            'message':
                "Expected value of type 'SpecialType' but got:"
                " NotSpecial('bar').",
            'locations': [(1, 3)], 'path': ['specials', 1]
        }])
