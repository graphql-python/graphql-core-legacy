...
# EmailType = GraphQLObjectType('Email', {
...
# InboxType = GraphQLObjectType('Inbox', {
#     'total': GraphQLField(
#         GraphQLInt, resolve=lambda inbox, _info: len(inbox['emails'])),
#     'unread': GraphQLField(
#         GraphQLInt, resolve=lambda inbox, _info: sum(
...
#             }]
#         },
#         'importantEmail': lambda _info, priority=None:
...
#     return send_important_email, await subscribe(
...
#             assert result == (None, [{
#                 'message': 'test error',
#                 'locations': [(3, 23)], 'path': ['importantEmail']}])

#         # Returning an error
#         def return_error(*args):
#             return TypeError('test error')

#         subscription_returning_error_schema = email_schema_with_resolvers(
#             return_error)
...
#         subscription_throwing_error_schema = email_schema_with_resolvers(
#             throw_error)
...
#         subscription_resolving_error_schema = email_schema_with_resolvers(
#             resolve_error)
...
#         subscription_rejecting_error_schema = email_schema_with_resolvers(
#             reject_error)
...
#             'unread': True}) is False  # No more listeners.
...
#         subscription = await subscribe(erroring_email_schema, parse("""
#             subscription {
#               importantEmail {
#                 email {
#                   subject
...
#         assert payload2 == ({'importantEmail': None}, [{
#             'message': 'Never leave',
#             'locations': [(3, 15)], 'path': ['importantEmail']}])
