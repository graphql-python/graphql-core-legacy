...
from graphql.type import (
    GraphQLDeprecatedDirective, GraphQLIncludeDirective,
    GraphQLSkipDirective, GraphQLEnumType, GraphQLObjectType,
    GraphQLInputObjectType, GraphQLInterfaceType, validate_schema)
...
def describe_schema_builder():
    ...
    def specifying_union_type_using_typename():
        ...
        assert graphql_sync(schema, query, root) == ({
            'fruits': [{'color': 'green'}, {'length': 5}]}, None)
    ...
    def correctly_assign_ast_nodes():
        ...
        assert print_ast(test_field.ast_node) == (
            'testField(testArg: TestInput): TestUnion')
        assert print_ast(test_field.args['testArg'].ast_node) == (
            'testArg: TestInput')
        assert print_ast(test_input.fields['testInputField'].ast_node) == (
            'testInputField: TestEnum')
        assert print_ast(test_enum.values['TEST_VALUE'].ast_node) == (
            'TEST_VALUE')
        assert print_ast(test_interface.fields['interfaceField'].ast_node) == (
            'interfaceField: String')
        assert print_ast(test_directive.args['arg'].ast_node) == (
            'arg: TestScalar')

    def root_operation_type_with_custom_names():
        schema = build_schema(dedent("""
            schema {
              query: SomeQuery
