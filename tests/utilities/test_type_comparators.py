...
from graphql.type import (
    GraphQLField, GraphQLFloat, GraphQLInt, GraphQLInterfaceType, GraphQLList,
    GraphQLNonNull, GraphQLObjectType, GraphQLOutputType, GraphQLSchema,
    ...
    def describe_is_equal_type():
        ...
        def lists_of_same_type_are_equal():
            assert is_equal_type(
                GraphQLList(GraphQLInt), GraphQLList(GraphQLInt)) is True
        ...
        def nonnull_of_same_type_are_equal():
            assert is_equal_type(
                GraphQLNonNull(GraphQLInt), GraphQLNonNull(GraphQLInt)) is True

        def nonnull_is_not_equal_to_nullable():
            assert is_equal_type(
                GraphQLNonNull(GraphQLInt), GraphQLInt) is False
    ...
    def describe_is_type_sub_type_of():
        ...
        def same_reference_is_subtype():
            assert is_type_sub_type_of(
                test_schema(), GraphQLString, GraphQLString) is True

        def int_is_not_subtype_of_float():
            assert is_type_sub_type_of(
                test_schema(), GraphQLInt, GraphQLFloat) is False

        def non_null_is_subtype_of_nullable():
            assert is_type_sub_type_of(
                test_schema(), GraphQLNonNull(GraphQLInt), GraphQLInt) is True

        def nullable_is_not_subtype_of_non_null():
            assert is_type_sub_type_of(
                test_schema(), GraphQLInt, GraphQLNonNull(GraphQLInt)) is False
        ...
        def implementation_is_subtype_of_interface():
            iface = GraphQLInterfaceType('Interface', {
                'field': GraphQLField(GraphQLString)})
