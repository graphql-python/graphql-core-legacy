...
from graphql.type import (
    GraphQLSchema, GraphQLDirective, GraphQLDeprecatedDirective,
    GraphQLIncludeDirective, GraphQLSkipDirective)
from graphql.utilities import (
    BreakingChangeType, DangerousChangeType,
    build_schema, find_breaking_changes, find_dangerous_changes)
from graphql.utilities.find_breaking_changes import (
    find_removed_types, find_types_that_changed_kind,
    find_fields_that_changed_type_on_object_or_interface_types,
    find_fields_that_changed_type_on_input_object_types,
    find_types_removed_from_unions, find_values_removed_from_enums,
    find_arg_changes, find_interfaces_removed_from_object_types,
    find_removed_directives, find_removed_directive_args,
    find_added_non_null_directive_args, find_removed_locations_for_directive,
    find_removed_directive_locations, find_values_added_to_enums,
    find_interfaces_added_to_object_types, find_types_added_to_unions)
...
def describe_find_breaking_changes():
    ...
    def should_detect_interfaces_removed_from_types():
        ...
        assert find_interfaces_removed_from_object_types(
    ...
    def should_detect_all_breaking_changes():
        ...
        assert find_breaking_changes(
            old_schema, new_schema) == expected_breaking_changes
    ...
    def should_detect_locations_removed_from_a_directive():
        d1 = GraphQLDirective('Directive Name', locations=[
            DirectiveLocation.FIELD_DEFINITION, DirectiveLocation.QUERY])

        d2 = GraphQLDirective('Directive Name', locations=[
            DirectiveLocation.FIELD_DEFINITION])

        assert find_removed_locations_for_directive(d1, d2) == [
...
def describe_find_dangerous_changes():
    ...
    def should_detect_interfaces_added_to_types():
        ...
        assert find_interfaces_added_to_object_types(
            old_schema, new_schema) == [
