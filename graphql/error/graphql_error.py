...
class GraphQLError(Exception):
    ...
    def __init__(
        self,
        message,  # type: str
        nodes=None,  # type: Union[Sequence[Node], Node]
        source=None,  # type: Source
        positions=None,  # type: Sequence[int]
        path=None,  # type: Sequence[Union[str, int]]
        original_error=None,  # type: Exception
        extensions=None,  # type: Dict[str, Any]
    ):
        # type: (...) -> None
    ...
    def __repr__(self):
        ...
        if self.locations:
            args.append("locations={!r}".format(self.locations))
        if self.path:
            args.append("path={!r}".format(self.path))
        if self.extensions:
