...
def print_error(error):
    ...
    """
    printed_locations = []  # type: List[str]
    print_location = printed_locations.append
    if error.nodes:
        for node in error.nodes:  # type: ignore
            if node.loc:
                print_location(
...
def highlight_source_at_location(source, location):
    ...
    return "{} ({}:{})\n".format(
        ...
        [
            ("{}: ".format(line_num - 1), get_line(line_index - 1)),
            ("{}: ".format(line_num), get_line(line_index)),
            ("", " " * (column_num - 1) + "^"),
            ("{}: ".format(line_num + 1), get_line(line_index + 1)),
        ]
    )
...
def print_prefixed_lines(lines):
